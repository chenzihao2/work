...
class LoginRespository
{
     ...
     * 依赖注入
     */
    public function __construct(hl_user $hl_user,hl_channel $hl_channel)
    {
        $this->model = $hl_user;
        $this->channel = $hl_channel;
    }
   ...
   public function phoneLogin($phone){
           $user_info=$this->model->userInfo(['phone'=>$phone]);
           if (!$user_info) {
                return [];
           }
    ...
    * 注册
    */
   public function createUser($i_data){

       $cid=$this->channel->addChannel($i_data);
       $i_data['cid']=$cid;
       $user_id = $this->model->addUser($i_data);
       if(!$cid || !$user_id){
           return ['code'=>1000110,'msg'=>'注册失败'];
       }
       $user_info = $user_info=$this->model->userInfo(['user_id'=>$user_id]);
       $user_info['is_reg']=1;
       return $user_info;
   }
   ...
   public function updateUser($user_id,$data){
       $userInfo=$this->model->userInfo(['user_id'=>$user_id]);
       $resUser=$this->model->userUpdate($user_id,$data);
       if(isset($data['phone'])){
           $data['mobile']=$data['phone'];
           unset($data['phone']);
       }
       $resChannel=$this->channel->channelUpdate($userInfo['cid'],$data);
       if(!$resUser || !$resChannel){
           return false;
       }
       return true;
    ...
    */
    ...
    public function insertChannel($user_info,$target,$platform,$channel,$device){
        ...
        $res=hl_login_channel::insertChannel($channel_data);
        return $res;
    }
