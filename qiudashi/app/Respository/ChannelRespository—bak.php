...
class ChannelRespository
{
     ...
     * 依赖注入
     */
    public function __construct(Order $order,User $user,UsersChannel $channel)
    {
        $this->model = $order;
        $this->user = $user;
    ...
    public function lists($startTime,$endTime,$channel='',$platform='',$page=1,$pageSize=20,$order_type=100){
        ...
        */
        ...
        $sql="select count(*) as counts from (select * from users_channel group by DATE_FORMAT(created_at,'%Y-%m-%d'),platform,channel) temps where $condtion";
    ...
    public function listsSum($startTime,$endTime,$channel='',$platform='',$page=1,$pageSize=20,$order_type=100){
        ...
        $sql="select count(*) as counts from (select * from users_channel group by DATE_FORMAT(created_at,'%Y-%m-%d'),platform) temps where $condtion";
    ...
    public function registerNum($startTime,$endTime,$channel='',$platform=''){
        $where[]=['users.created_at','>=',$startTime];
        $where[]=['users.created_at','<=',$endTime];

        if($channel){
            $where[]=['users_channel.channel','=',$channel];
        }
        //if($platform){
        $where[]=['users_channel.platform','=',$platform];
        //}

        // dump($where);
//DB::connection()->enableQueryLog();
        $res=$this->user->leftJoin('users_channel','3.cid','users.cid')->where($where)->count();
    ...
    public function thisDayTotal($startTime,$endTime,$channel='',$platform='',$order_type=100){
        ...
        $user_id=$this->user
            ->leftJoin('users_channel','users_channel.user_id', 'users.user_id')->where($where)
    ...
    public function union($regSstartTime,$resEndTime,$startTime,$endTime,$channel,$platform='',$order_type=100){
        $where[]=['users.created_at','>=',$regSstartTime];
        $where[]=['users.created_at','<=',$resEndTime];
        if($channel){
            $where[]=['users_channel.channel','=',$channel];
        }
        if($platform){
        ...
        $user_id=$this->user
            ->leftJoin('users_channel', 'users_channel.cid', 'users.cid')->where($where)
            ->pluck('users.user_id')->toArray();
