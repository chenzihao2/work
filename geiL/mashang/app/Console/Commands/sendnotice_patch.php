...
class sendnotice_patch extends Command {

    protected $signature = 'sendnotice_patch';

    protected $description = 'sendnotice_patch';
    ...
    public function handle() {
        ...
        //while ($sid = Redis::rpop($redisKey)) {
            //获取料详情
        $sourceInfos = source::select('source.sid', 'uid', 'nickname', 'title', 'source.createtime', 'is_notice')->LeftJoin('client', 'source.uid', 'client.id')->where('source.is_notice', 0)
->where('source.createtime', '>', '2019-12-29 04:00:00')->where('source.createtime', '<', '2019-12-29 11:30:00')->get();
//var_dump($sourceInfos);die;
