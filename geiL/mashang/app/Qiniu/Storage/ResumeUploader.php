...
final class ResumeUploader
{
    ...
    public function __construct(
        ...
        list($accessKey, $bucket, $err) = Common::explodeUpToken($upToken);
        if ($err != null) {
    ...
    public function upload($fname)
    {
        ...
        while ($uploaded < $this->size) {
            ...
            if ($response->statusCode < 0) {
                list($accessKey, $bucket, $err) = Common::explodeUpToken($this->upToken);
                if ($err != null) {
                    return array(null, $err);
                }
    ...
    private function fileUrl($fname)
    {
        ...
        $url .= '/mimeType/' . Common::base64_urlSafeEncode($this->mime);
        if ($this->key != null) {
            $url .= '/key/' . Common::base64_urlSafeEncode($this->key);
        }
        $url .= '/fname/' . Common::base64_urlSafeEncode($fname);
        if (!empty($this->params)) {
            foreach ($this->params as $key => $value) {
                $val = Common::base64_urlSafeEncode($value);
                $url .= "/$key/$val";
            }
        }
        return $url;
