...
*                                            ]
    ...
    public function listFiles(
        ...
        Common::setWithoutEmpty($query, 'prefix', $prefix);
        Common::setWithoutEmpty($query, 'marker', $marker);
        Common::setWithoutEmpty($query, 'limit', $limit);
        Common::setWithoutEmpty($query, 'delimiter', $delimiter);
...
*                                            ]
    ...
    public function listFilesv2(
        ...
        Common::setWithoutEmpty($query, 'limit', $limit);
        Common::setWithoutEmpty($query, 'delimiter', $delimiter);
        Common::setWithoutEmpty($query, 'skipconfirm', $skipconfirm);
...
*                                              ]
    ...
    {
        $path = '/stat/' . Common::entry($bucket, $key);
    ...
    {
        $path = '/delete/' . Common::entry($bucket, $key);
    ...
    {
        $from = Common::entry($from_bucket, $from_key);
        $to = Common::entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        if ($force === true) {
            $path .= '/force/true';
        }
    ...
    {
        $resource = Common::entry($bucket, $key);
        $encode_mime = Common::base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = Common::base64_urlSafeEncode($url);
        $to = Common::entry($bucket, $key);
    ...
    public static function buildBatchDeleteAfterDays($bucket, $key_day_pairs)
    {
        ...
        foreach ($key_day_pairs as $key => $day) {
            array_push($data, '/deleteAfterDays/' . Common::entry($bucket, $key) . '/' . $day);
        }
        return $data;
    }
    ...
    public static function buildBatchChangeMime($bucket, $key_mime_pairs)
    {
        ...
        foreach ($key_mime_pairs as $key => $mime) {
            array_push($data, '/chgm/' . Common::entry($bucket, $key) . '/mime/' . base64_encode($mime));
        }
        return $data;
    }
    ...
    public static function buildBatchChangeType($bucket, $key_type_pairs)
    {
        ...
        foreach ($key_type_pairs as $key => $type) {
            array_push($data, '/chtype/' . Common::entry($bucket, $key) . '/type/' . $type);
        }
        return $data;
    }
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . Common::entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket, $force)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = Common::entry($source_bucket, $from_key);
            $to = Common::entry($target_bucket, $to_key);
